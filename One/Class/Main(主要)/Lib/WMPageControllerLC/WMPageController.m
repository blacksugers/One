...
- (instancetype)initWithViewControllers:(NSArray *)classes andTheirTitles:(NSArray *)titles{
    if (self = [super init]) {
...
- (void)viewDidLoad {
    ...
    [self calculateSize];
    [self addScrollView];
    [self addMenuViewWith:self.menuY];
...
//    UIViewController *viewController = [[vcClass alloc] init];       _______________LC主要修改位置
    
    UIViewController *viewController = self.viewControllerClasses[index];   //改
...
//lichao
- (void)addMenuViewWith:(CGFloat)menuY{
    //改
    CGFloat y ;
    if (self.menuY) {
        y = menuY;
        ...
        y = 0;
    }                                                          // _______________lc主要增加位置
    CGRect frame = CGRectMake(0, y, self.view.frame.size.width, self.menuHeight);
